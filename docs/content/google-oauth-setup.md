...
Nexus IDP uses Google OAuth to authenticate users. Sign-in is restricted to `@<yourdomain.com>` accounts. There is no Google Workspace sync — users are managed in the Backstage database after their first sign-in.
...
4. If the email is not `@<yourdomain.com>`, sign-in is rejected
...
5. Authorized redirect URIs:
   ...
   - Production: `https://<your-domain>/api/auth/google/handler/frame`
...
```yaml
organization:
  domain: <yourdomain.com>   # Controls which email domain is allowed
...
2. Requires at least one verified `@<yourdomain.com>` email
...
AUTH_GITHUB_CALLBACK_URL=https://<your-domain>/api/auth/github/handler/frame
```
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| "Sign-in is restricted to @<yourdomain.com>" | User signed in with a personal Google account | User must use their `@<yourdomain.com>` Google account |
...
| GitHub: "no verified @<yourdomain.com> email" | GitHub account doesn't have org email added | User must add and verify their `@<yourdomain.com>` email in GitHub settings |
