...
Nexus IDP uses Google OAuth to authenticate users. Sign-in is restricted to `@yourcompany.com` accounts. There is no Google Workspace sync — users are managed in the Backstage database after their first sign-in.
...
4. If the email is not `@yourcompany.com`, sign-in is rejected
...
5. Authorized redirect URIs:
   ...
   - Production: `https://portal.yourcompany.io/api/auth/google/handler/frame`
...
```yaml
organization:
  domain: yourcompany.com   # Controls which email domain is allowed
...
2. Requires at least one verified `@yourcompany.com` email
...
AUTH_GITHUB_CALLBACK_URL=https://portal.yourcompany.io/api/auth/github/handler/frame
```
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| "Sign-in is restricted to @yourcompany.com" | User signed in with a personal Google account | User must use their `@yourcompany.com` Google account |
...
| GitHub: "no verified @yourcompany.com email" | GitHub account doesn't have org email added | User must add and verify their `@yourcompany.com` email in GitHub settings |
